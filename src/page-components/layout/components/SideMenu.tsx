...
import s from '@/page-components/layout/styles/sideMenu.module.css';
...
const SideMenu = (props: {
  ...
  return (
    <div className={s.customMenu}>
      {menuOptions.map((menu: any) => {
        if (menu.children?.length) {
          return (
            <div key={menu.key} className={s.subMenu}>
              <div
                className={cs(s.sub, {
                  [s['sub-open']]: openKeys.includes(menu.key),
                  [s['sub-actived']]: menu.children.some((it: any) => {
                    return (
                      it.key === selectedKey ||
              ...
              {openKeys.includes(menu.key) && (
                <div
                  className={cs(s['subMenu-wrapper'])}
                  ...
                  {menu.children.map((subMenu: any) => {
                    ...
                    return (
                      <div
                        key={subMenu.key}
                        className={cs(s.item, {
                          [s['item-selected']]: isActivitedSubMenu,
                        })}
        ...
        return (
          <div
            key={menu.key}
            className={cs(s.item, {
              [s['item-selected']]: isActivetedMenu,
            })}
            data-value={menu.key}

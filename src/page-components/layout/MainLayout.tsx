...
import {isAuthRoute} from '@/utils/authRoute'
...
export const MainLayout: React.FC<IProps> = props => {
  ...
  useEffect(() => {
    ...
    const authCookie = cookie.get('USER_COOKIES_NEED_CHANGE');
    ...
    if (authCookie) {
      // 查看该登录用户是否有访问此路由的权限
      const isAuthRouterPath = isAuthRoute(role, pathname);
      if (!notFindPage && !isNoLogin && !isAuthRouterPath) {
        // 用户角色无权限重定向至403页面
